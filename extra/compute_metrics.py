...
def rgb_lpips(np_gt, np_im, net_name, device):
    ...
    # 转为 tensor，添加 batch 维度
    gt = torch.from_numpy(np_gt).permute(2, 0, 1).unsqueeze(0).contiguous().to(device)
    im = torch.from_numpy(np_im).permute(2, 0, 1).unsqueeze(0).contiguous().to(device)

    with torch.no_grad():
        torch.cuda.empty_cache()  # 可选：减少显存碎片
        dist = __LPIPS__[net_name](gt, im, normalize=True)
