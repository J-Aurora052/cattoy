...
class MLPRender_PE(torch.nn.Module):
    ...
    def forward(self, pts, viewdirs, features):
        ...
        if self.pospe > 0:
            indata += [pts, positional_encoding(pts, self.pospe)]
        else:
            indata += [pts]
...
class TensorBase(torch.nn.Module):
    def __init__(self, aabb, gridSize, device, density_n_comp=8, appearance_n_comp=24, app_dim=27,
        ...
        print(">>> density_n_comp input:", density_n_comp)
        self.matMode = [[0, 1], [0, 2], [1, 2]]
        self.vecMode = [2, 1, 0]
        self.comp_w = [1, 1, 1]

        # 安全处理组件数
        self.density_n_comp = 8 if density_n_comp is None else density_n_comp
        self.app_n_comp = 24 if appearance_n_comp is None else appearance_n_comp

        if isinstance(self.density_n_comp, int):
            self.density_n_comp = [self.density_n_comp] * len(self.vecMode)
        if isinstance(self.app_n_comp, int):
            self.app_n_comp = [self.app_n_comp] * len(self.vecMode)
